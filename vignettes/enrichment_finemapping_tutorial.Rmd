...
`prepare_susie_data_with_torus_result()` takes a parameter called `fdr_thresh` which is the FDR associated with each chunk. 
...
```{r}
susie.df <- prepare_susie_data_with_torus_result(sumstats = gwas, torus_prior = torus.prior, torus_fdr = torus.fdr, fdr_thresh = 1)
```

We see we have a new column called `torus_prior` 

```{r}
susie.df[,c('chr','pos','beta','se','snp','pval','zscore','torus_prior')]
```
...
```{r}
gwas_finemapped[,c('chr','pos','beta','se','snp','pval','zscore','torus_prior','susie_pip')]
```
