---
title: "Quick tutorial for enrichment analysis and fine-mapping using functional priors"
...
Welcome to this tutorial where we perform fine-mapping on a small GWAS dataset that is included with this package. Try to reproduce this analysis to ensure everything is installed properly.
...
We begin by attaching our `bigsnpr` object. 
This is our reference panel of genotypes which we use to compute LD between SNPs. 

If you are in the He lab, you can just use the following chunk. 
...
We also need reference LD blocks. We included reference LD blocks from European 1KG. 
```{r}
# load LD blocks
...
```{r}
# Testing GWAS summary statistics
...
```{r}
head(gwas[,c('chr','pos','beta','se','snp','pval','zscore')])
```
...
`run_torus()` with `torus_option = "dump_prior"` returns a list with: 
...
We run `run_torus()` with `torus_option = "qtl"` to get the probability of each chunk containing a causal variant. 
...
```{r}
head(gwas_finemapped[,c('chr','pos','beta','se','snp','pval','zscore','torus_pip','susie_pip')])
```
