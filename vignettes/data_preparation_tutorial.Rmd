...
_* You can skip this if you only need to run enrichment analysis, 
or you have LD matrices, or if you only need to run 
...
(optionally) match alleles in GWAS SNPs with the reference panel from
the `bigSNP` object, and (optionally) add locus ID from the `LD_Blocks`.
...
_* You do not need to specify `bigSNP` and `LD_Blocks` if you only need to run enrichment analysis._
