...
You can skip this if you only need to run enrichment analysis or 
other analysis (e.g.gene mapping) with precomputed fine-mapping result.
...
We provided LD blocks from `LDetect` for 1000 Genomes (1KG) European population (in hg19).
The data frame has four columns: 
chromosome, start and end positions (in hg19), and 
the LD block indices.
```{r load-LD-blocks}
LD_blocks <- readRDS(system.file('extdata', 'LD.blocks.LDetect.1KG.EUR.hg19.rds', package='mapgen'))
...
# Read in GWAS summary statistics using vroom package
library(vroom)
...
from the bigSNP object, and add locus ID from the LD Blocks.
