...
We included gene annotations (hg19) in the package.
...
with the following columns, and save as a GRanges object.
  ...
  * promoter_chr: chromosome of the gene promoter
...
Include ABC, PC-HiC and nearby interactions in the "enhancer loop" category.
```{r}
...
* `exon.weight`: weight for exon (and promoter) categories (default = 1).
* `loop.weight`: weight for chromatin loop categories (default = 1).

```{r run-gene-mapping, message=FALSE, warning=FALSE}
gene.mapping.res <- compute_gene_pip(finemapstats, genomic.annots, 
                                     intron.mode = FALSE, d0 = 50000,
                                     exon.weight = 1, loop.weight = 1)
