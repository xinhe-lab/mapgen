...
#' @export
run_finemapping <- function(sumstats, R, bigSNP, priortype = c('torus', 'uniform'),
  ...
  for(i in seq_along(chunks)){
    ...
    susie_res[[as.character(z)]] <- run_susie(susie.df, R = R, bigSNP = bigSNP,
                                              L = L, useprior = useprior,
...
#' @export
run_susie <- function(sumstats, R, bigSNP, locus, L=1, useprior,
